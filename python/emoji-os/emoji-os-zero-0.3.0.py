# -*- coding:utf-8 -*-
# Emoji OS Zero v0.3.0 - Enhanced with BLE Controller functionality
import LCD_1in44
import time
import threading
import asyncio
...
def send_emoji_to_pico(menu_val, pos_val, neg_val):
    ...
    def send_command():
        try:
            ...
            # Send the command
            loop.run_until_complete(ble_controller.send_emoji_command(menu_val, pos_val, neg_val))
...
def init_ble_connection():
    ...
    def connect():
        try:
            ...
            # Scan and connect
            if loop.run_until_complete(ble_controller.scan_for_device(timeout=5)):
                loop.run_until_complete(ble_controller.connect_to_device())
            
        except Exception as e:
            print(f"BLE initialization error: {e}")
    
    # Start BLE connection in background
    ble_thread = threading.Thread(target=connect)
...
try:
    ...
    try:
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        loop.run_until_complete(ble_controller.disconnect())
    except:
        pass
    disp.module_exit()
