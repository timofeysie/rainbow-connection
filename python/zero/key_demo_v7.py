...
selected_side = "left"  # "left" or "right"
selected_emoji = 0  # 0-3 for the 4 emoji positions
...
def handle_menu_navigation():
    global selected_menu, selected_side, selected_emoji
    
    # Center button - cycle through main menu
    if check_button_press('center', disp.GPIO_KEY_PRESS_PIN):
        selected_menu = (selected_menu + 1) % len(menu_items)
        selected_side = "left"  # Reset to left side
        selected_emoji = 0  # Reset to top emoji
        print(f"CENTER - Main Menu: {menu_items[selected_menu]}")
        time.sleep(0.2)
        return True
    
    # Left button - select left side
    if check_button_press('left', disp.GPIO_KEY_LEFT_PIN):
        selected_side = "left"
        selected_emoji = 0  # Reset to top
        print(f"LEFT - Selected left side, emoji 0")
        time.sleep(0.2)
        return True
    
    # Right button - select right side
    if check_button_press('right', disp.GPIO_KEY_RIGHT_PIN):
        selected_side = "right"
        selected_emoji = 0  # Reset to top
        print(f"RIGHT - Selected right side, emoji 0")
        time.sleep(0.2)
        return True
    
    # Up button - cycle through emojis (up)
    if check_button_press('up', disp.GPIO_KEY_UP_PIN):
        selected_emoji = (selected_emoji - 1) % 4
        print(f"UP - {selected_side} side, emoji {selected_emoji}")
        time.sleep(0.2)
        return True
    
    # Down button - cycle through emojis (down)
    if check_button_press('down', disp.GPIO_KEY_DOWN_PIN):
        selected_emoji = (selected_emoji + 1) % 4
        print(f"DOWN - {selected_side} side, emoji {selected_emoji}")
        time.sleep(0.2)
        return True
...
try:
    while True:
        ...
        # Left side emojis
        for i in range(4):
            show_selection = (selected_side == "left" and selected_emoji == i)
            draw_emoji(draw, smiley_matrix, color_map, 1.5, 5, emoji_y_positions[i], show_selection)
        
        # Right side emojis
        for i in range(4):
            show_selection = (selected_side == "right" and selected_emoji == i)
